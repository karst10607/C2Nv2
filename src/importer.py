...
def main(argv: Optional[list] = None):
    ...
    # Keep tunnel alive for Notion to fetch images (default 120s)
    if not args.dry_run:
        import time, os
        keep_alive = int(os.environ.get('IMAGE_TUNNEL_KEEPALIVE_SEC', '120'))
        print(f"\n[green]Keeping tunnel alive for {keep_alive} seconds for Notion to cache images...[/green]")
